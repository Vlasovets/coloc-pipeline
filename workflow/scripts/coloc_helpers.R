...
make_vcf <- function(GWASfile, chrom=NULL, pos=NULL, nea=Allele2, ea=Allele1, snp, ea_af=Freq1, effect=Effect, se=StdErr, pval=p, variant_ann, GWAS_n=GWAS_n, WantToLiftOver=TRUE, output=NULL, ch_path = "/lustre/groups/itg/teams/zeggini/projects/fungen-oa/analyses/Ana_coloc_mr/FunctionsAndData/hg19ToHg38.over.chain"){
    ...
    # Check for and fix duplicate column names
    if(any(duplicated(colnames(vcf)))){
        dup_cols <- colnames(vcf)[duplicated(colnames(vcf))]
        print(paste0("WARNING: Found duplicate column names: ", paste(dup_cols, collapse=", ")))
        print("Making column names unique...")
        colnames(vcf) <- make.unique(colnames(vcf), sep="_")
        print(paste0("New column names: ", paste(colnames(vcf), collapse=", ")))
    }
...
lifOverFunction_vcf <- function(regions, ch_path){
  ...
  # Return only the essential columns to avoid duplicates
  return(res_ggr)
}
